...
feature in conjunction with grid search."
    ...
    learning_rates = [0.001, 0.0005, 0.0001, .00005]
    gradient_clipping = [2.0, 1.0, 0.5]
    weight_decay = [.025, .010, .001]
    ...
    for lr, gradient_clipping, weight_decay in param_combinations:
        # Prepare seperate config objects to pass
        longnet_config = copy.deepcopy(config)
        longnet_config.lr = lr
        longnet_config.gradient_clipping = gradient_clipping
        longnet_config.weight_decay = weight_decay
        longnet_config.model_type = "longnet"
        
        # retnet_config = copy.deepcopy(config)
        # retnet_config.lr = lr
        # retnet_config.gradient_clipping = gradient_clipping
        # retnet_config.weight_decay = weight_decay
        # retnet_config.model_type = "retnet"

        # transformer_config = copy.deepcopy(config)
        # transformer_config.lr = lr
        # transformer_config.gradient_clipping = gradient_clipping
    ...
    # similarity_retnet_transformer[(lr, gradient_clipping, weight_decay)] = similarity_score_1
    # similarity_retnet_longnet[(lr, gradient_clipping, weight_decay)] = similarity_score_2
    # similarity_longnet_transformer[(lr, gradient_clipping, weight_decay)] = similarity_score_3
...
if __name__ == "__main__":
    ...
    config.slurm_task = int(slurm_task)
