...
def generate(config: Struct, alt_pattern: str = "NO ALT. PATTERN PROVIDED"):
    """
    Args:
        config (Struct): A Struct object with all configuration fields.
    """
    print('Preparing model devices...')
    model, device = 1,2 #prepare_model_device(config)
...
def generate_text(
    ...
    for prompt_list in prompts:
        print(prompt_list)
        prompt_list_short = [prompt[:prompt.rfind(" ")] for prompt in prompt_list]  # cut off last word in prompts
        nshots =  [prompt + " " + alt_pattern for prompt in prompt_list_short[:-1]]  # Add alternate ending to idioms
        lm_prompt = prompt_list_short[-1]  # Prompt for LM
    ...
    [print(e, "\t", full_prompt, "\n\n") for e, full_prompt in enumerate(full_prompts)]  # Sanity Check
...
if __name__ == "__main__":
    ...
    alternate_pattern = "webbing."

    generate(config, alt_pattern=alternate_pattern)
